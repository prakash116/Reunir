...
const API_URL = 'http://localhost:8585/coach';
...
const NewRegisterCoachs = () => {
  ...
  const fetchCoaches = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data && Array.isArray(result.data.coaches)) {
  ...
  const filteredCoaches = useMemo(() => {
    ...
    return coaches.filter(coach =>
      ['name', 'email', 'number', 'city', 'profession', 'sportDiscipline', 'experienceAsCoach', 'overallExperience'].some(field =>
        String(coach[field] || '').toLowerCase().includes(lowercasedSearch)
      )
    );
  }, [coaches, searchTerm]);
  ...
  const sortedCoaches = useMemo(() => {
    ...
    return sortableCoaches.sort((a, b) => {
  ...
  }, [filteredCoaches, sortConfig]);

  // Paginate coaches
  const paginatedCoaches = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    return sortedCoaches.slice(startIndex, startIndex + itemsPerPage);
  ...
  const getExperienceBadgeColor = useCallback((experience) => {
  ...
  const tableHeaders = useMemo(() => [
    ...
    { key: 'experienceAsCoach', label: 'Coach Exp.', sortable: true, mobile: true },
    { key: 'overallExperience', label: 'Overall Exp.', sortable: true, mobile: false },
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
        ...
        >
          Recently registered coaches list
        </motion.p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
          <div className="flex-1 w-full sm:max-w-md">
            <div className="relative">
              <input
                type="text"
                placeholder="Search by name, email, phone, city, profession, sport, experience..."
                value={searchTerm}
          ...
          <div className="flex items-center space-x-2 text-sm text-gray-600">
            <span>Total:</span>
            <span className="font-semibold text-blue-600">{sortedCoaches.length}</span>
            <span>coaches</span>
          </div>
        </div>
      </motion.div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex flex-col items-end space-y-1">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          ...
                          {coach.sportperson ? 'Athlete' : 'Coach'}
                        </span>
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getExperienceBadgeColor(coach.experienceAsCoach)}`}>
                          {coach.experienceAsCoach || 'N/A'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-gray-900">{coach.number}</p>
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <p className="text-gray-900 truncate">{coach.email}</p>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <span className="text-sm font-medium text-gray-500">Profession</span>
                        <p className="text-gray-900">{coach.profession || 'N/A'}</p>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-gray-500">City</span>
                        <p className="text-gray-900">{coach.city || 'N/A'}</p>
                      </div>
                    </div>

                    <div>
                      <span className="text-sm font-medium text-gray-500">Sport Discipline</span>
                      <p className="text-gray-900 font-medium">{coach.sportDiscipline || 'N/A'}</p>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <span className="text-sm font-medium text-gray-500">Coach Exp.</span>
                        <p className="text-gray-900">{coach.experienceAsCoach || 'N/A'}</p>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-gray-500">Overall Exp.</span>
                        <p className="text-gray-900">{coach.overallExperience || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                </motion.div>
                ...
                paginatedCoaches.map((coach, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="text-sm font-semibold text-gray-900">{coach.name}</div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="text-sm text-gray-900 max-w-xs truncate" title={coach.email}>
                        {coach.email}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                      {coach.profession || 'N/A'}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                      {coach.city || 'N/A'}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                      <div className="max-w-[150px] truncate" title={coach.sportDiscipline}>
                        {coach.sportDiscipline || 'N/A'}
                      </div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getExperienceBadgeColor(coach.experienceAsCoach)}`}>
                        {coach.experienceAsCoach || 'N/A'}
                      </span>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                      {coach.overallExperience || 'N/A'}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
              <div className="text-sm text-gray-700">
                ...
                <span className="font-semibold">
                  {Math.min(currentPage * itemsPerPage, sortedCoaches.length)}
                </span> of{' '}
                <span className="font-semibold">{sortedCoaches.length}</span> coaches
              </div>
...
export default NewRegisterCoachs;